...
router.post('/entradas', (req, res) => {
  ...
  const r = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(req.user.empresaUserId, valor, descricao || '', dataStr, status || 'Recebido', forma_pagamento || '', cliente_id || null)
...
router.put('/entradas/:id', (req, res) => {
  ...
  `).run(valor, descricao, dataStr, status, forma_pagamento, cliente_id || null, req.params.id, req.user.empresaUserId)
...
router.delete('/entradas/:id', (req, res) => {
  db.prepare('DELETE FROM entradas WHERE id = ? AND user_id = ?').run(req.params.id, req.user.empresaUserId)
  res.json({ ok: true })
})

router.get('/gastos', (req, res) => {
  const rows = db.prepare('SELECT * FROM gastos WHERE user_id = ? ORDER BY data DESC').all(req.user.empresaUserId)
  res.json(rows)
})
...
router.post('/gastos', (req, res) => {
  ...
  const r = db.prepare('INSERT INTO gastos (user_id, valor, descricao, categoria, data) VALUES (?, ?, ?, ?, ?)')
    .run(req.user.empresaUserId, valor, descricao || '', categoria || '', dataStr)
...
router.put('/gastos/:id', (req, res) => {
  ...
  db.prepare('UPDATE gastos SET valor = COALESCE(?, valor), descricao = COALESCE(?, descricao), categoria = COALESCE(?, categoria), data = COALESCE(?, data) WHERE id = ? AND user_id = ?')
    .run(valor, descricao, categoria, dataStr, req.params.id, req.user.empresaUserId)
  const row = db.prepare('SELECT * FROM gastos WHERE id = ?').get(req.params.id)
  res.json(row || {})
})

router.delete('/gastos/:id', (req, res) => {
  db.prepare('DELETE FROM gastos WHERE id = ? AND user_id = ?').run(req.params.id, req.user.empresaUserId)
  res.json({ ok: true })
})

router.get('/custos', (req, res) => {
  const rows = db.prepare('SELECT * FROM custos WHERE user_id = ? ORDER BY vencimento DESC').all(req.user.empresaUserId)
  res.json(rows)
})
...
router.post('/custos', (req, res) => {
  ...
  const r = db.prepare('INSERT INTO custos (user_id, valor, descricao, vencimento) VALUES (?, ?, ?, ?)')
    .run(req.user.empresaUserId, valor, descricao || '', venc)
  const row = db.prepare('SELECT * FROM custos WHERE id = ?').get(r.lastInsertRowid)
  res.status(201).json(row)
})

router.put('/custos/:id/pago', (req, res) => {
  db.prepare('UPDATE custos SET pago = 1 WHERE id = ? AND user_id = ?').run(req.params.id, req.user.empresaUserId)
  const row = db.prepare('SELECT * FROM custos WHERE id = ?').get(req.params.id)
  res.json(row || {})
})

router.delete('/custos/:id', (req, res) => {
  db.prepare('DELETE FROM custos WHERE id = ? AND user_id = ?').run(req.params.id, req.user.empresaUserId)
  res.json({ ok: true })
})

router.get('/poupanca', (req, res) => {
  const rows = db.prepare('SELECT * FROM poupanca WHERE user_id = ? ORDER BY data DESC').all(req.user.empresaUserId)
  res.json(rows)
})
...
router.post('/poupanca', (req, res) => {
  ...
  const r = db.prepare('INSERT INTO poupanca (user_id, valor, descricao, data, tipo) VALUES (?, ?, ?, ?, ?)')
    .run(req.user.empresaUserId, valor, descricao || '', dataStr, tipo || 'Poupança')
  const row = db.prepare('SELECT * FROM poupanca WHERE id = ?').get(r.lastInsertRowid)
  res.status(201).json(row)
})

router.delete('/poupanca/:id', (req, res) => {
  db.prepare('DELETE FROM poupanca WHERE id = ? AND user_id = ?').run(req.params.id, req.user.empresaUserId)
  res.json({ ok: true })
})
