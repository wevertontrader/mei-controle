...
router.get('/', (req, res) => {
  const rows = db.prepare('SELECT * FROM produtos WHERE user_id = ? ORDER BY nome').all(req.user.id)
  res.json(rows)
})
...
router.post('/', (req, res) => {
  ...
  const r = db.prepare('INSERT INTO produtos (user_id, nome, codigo, preco_custo, preco, estoque_atual, unidade) VALUES (?, ?, ?, ?, ?, ?, ?)')
    .run(req.user.id, nome || '', codigo || '', preco_custo || 0, preco || 0, estoque_atual || 0, unidade || 'UN')
...
router.put('/:id', (req, res) => {
  ...
  db.prepare('UPDATE produtos SET nome = COALESCE(?, nome), codigo = COALESCE(?, codigo), preco_custo = COALESCE(?, preco_custo), preco = COALESCE(?, preco), estoque_atual = COALESCE(?, estoque_atual), unidade = COALESCE(?, unidade) WHERE id = ? AND user_id = ?')
    .run(nome, codigo, preco_custo, preco, estoque_atual, unidade, req.params.id, req.user.id)
  const row = db.prepare('SELECT * FROM produtos WHERE id = ?').get(req.params.id)
  res.json(row || {})
})

router.delete('/:id', (req, res) => {
  db.prepare('DELETE FROM produtos WHERE id = ? AND user_id = ?').run(req.params.id, req.user.id)
  res.json({ ok: true })
})
