...
router.get('/', (req, res) => {
  const rows = db.prepare('SELECT * FROM tarefas WHERE user_id = ? ORDER BY data_limite ASC, id DESC').all(req.user.empresaUserId)
  res.json(rows)
})
...
router.post('/', (req, res) => {
  ...
  const r = db.prepare('INSERT INTO tarefas (user_id, titulo, descricao, data_limite, hora, tipo) VALUES (?, ?, ?, ?, ?, ?)')
    .run(req.user.empresaUserId, tituloVal, descricao || '', dataStr, hora || null, tipo || 'Trabalho')
...
router.put('/:id', (req, res) => {
  ...
  db.prepare('UPDATE tarefas SET titulo = COALESCE(?, titulo), descricao = COALESCE(?, descricao), data_limite = COALESCE(?, data_limite), hora = COALESCE(?, hora), tipo = COALESCE(?, tipo), concluida = COALESCE(?, concluida) WHERE id = ? AND user_id = ?')
    .run(titulo, descricao, dataStr, hora, tipo, concluida, req.params.id, req.user.empresaUserId)
  const row = db.prepare('SELECT * FROM tarefas WHERE id = ?').get(req.params.id)
  res.json(row || {})
})

router.delete('/:id', (req, res) => {
  db.prepare('DELETE FROM tarefas WHERE id = ? AND user_id = ?').run(req.params.id, req.user.empresaUserId)
  res.json({ ok: true })
})
