...
router.post('/movimentacoes', (req, res) => {
  ...
  const r = db.prepare('INSERT INTO estoque_movimentacoes (user_id, produto_id, quantidade, tipo, descricao, data) VALUES (?, ?, ?, ?, ?, ?)')
    .run(req.user.id, produto_id, quantidade, tipo || 'entrada', descricao || '', dataStr)

  const qtd = Number(quantidade)
  const produto = db.prepare('SELECT estoque_atual FROM produtos WHERE id = ? AND user_id = ?').get(produto_id, req.user.id)
  if (produto) {
    const novoEstoque = (tipo === 'entrada') ? produto.estoque_atual + qtd : Math.max(0, produto.estoque_atual - qtd)
    db.prepare('UPDATE produtos SET estoque_atual = ? WHERE id = ? AND user_id = ?').run(novoEstoque, produto_id, req.user.id)
  }
